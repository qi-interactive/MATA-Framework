...
class ItemOrderableBehavior extends \yii\base\Behavior implements ItemOrderableInterface 
{
	...
	public function next() 
	{
        ...
        if(!empty($currentItemOrder)) {
        	return $this->prepareQuery($class, 'mata_itemorder.Order > :currentOrder', [':currentOrder' => $currentItemOrder->Order])->one();
        }
	...
	public function last() 
	{
		...
		return $this->prepareQuery($class, 'mata_itemorder.Order = MAX(mata_itemorder.Order)')->one(); 
	}
	...
	protected function prepareQuery($class, $whereCondition = false, $whereParams = []) 
	{
        ...
        $query->orderBy('mata_itemorder.Order ASC');
        return $query;    
	}
