	...
	public static function generateFromTableName($tableName, $omitId = true) {
		if(!empty($tableName)) {
    		...
    		if(!empty($tableSchema->columns)) {
                ...
                $dynamicModel = new DynamicModel($fields);
                foreach($fields as $attribute) {
                	$dynamicModel->setAttributeLabel($attribute, $attribute);
                }
                $dynamicModel->setTableName($tableName);

                $dynamicModel = self::generateRulesFromTable($dynamicModel, $tableSchema, $omitId);
